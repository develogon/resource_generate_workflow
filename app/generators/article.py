...
class ArticleGenerator(BaseGenerator):
    ...
    def prepare_prompt(self, structure: Dict, **kwargs) -> str:
        """記事生成用プロンプトを準備する
        ...
        """
        # システムプロンプト（役割設定）を取得
        system_prompt = self.get_system_prompt('article')
        
        # メッセージプロンプト（具体的な指示）を取得し、必要に応じて変数を置換
        message_prompt = self.get_message_prompt('article')
        
        # セクション情報の抽出と整形
        sections = ', '.join([section.get('title', 'セクションタイトルなし') for section in structure.get('sections', [])])
        message_prompt = message_prompt.replace('{{SECTIONS}}', sections)
...
"""
    ...
    async def generate(self, structure: Dict, output_path: Optional[str] = None) -> str:
        """記事を生成する
        ...
        if output_path is None:
            ...
            output_path = self.get_output_path(structure, level, 'article.md')
    ...
    def generate_article(self, structure: Dict, output_path: Optional[str] = None) -> str:
        """記事を生成する（同期版）
        ...
        try:
            ...
            return loop.run_until_complete(self.generate(structure, output_path))
        except RuntimeError:
            # イベントループがない場合、新しく作成して実行
            return asyncio.run(self.generate(structure, output_path)) 